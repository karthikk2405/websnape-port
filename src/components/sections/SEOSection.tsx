...
export const SEOSection: React.FC = () => {
  return (
    <section id="seo" className="py-24 px-6 md:px-12 max-w-7xl mx-auto border-t border-white/10 relative">
      {/* Section Header */}
      <div className="text-center max-w-3xl mx-auto mb-16">
        <span className="text-xs font-mono font-bold uppercase tracking-widest text-[#00F0FF] mb-3 block">
          SEO MANAGEMENT / PRODUCT 03
        </span>
        <h2 className="rynd-title text-5xl sm:text-7xl text-white mb-6">
          Get found.<br />
          <span className="gradient-text">Stay found.</span>
        </h2>
        <p className="text-lg text-[#8A8D93] leading-relaxed font-medium">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-16">
        {RESULTS.map((r, i) => (
          <div key={i} className="glass-panel p-8 text-center bg-[#111114] border-white/10 hover:border-[#00F0FF]/30 transition-all duration-300">
            <p className="text-5xl font-bold text-[#00F0FF] mb-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{r.value}</p>
            <p className="text-sm font-bold text-white mb-1">{r.label}</p>
            <p className="text-xs text-[#A9B1BD]">{r.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
        {SEO_FEATURES.map((f, i) => {
          const Icon = f.icon;
          return (
            <div key={i} className="glass-card p-6 rounded-2xl border border-white/10 bg-[#111114] group">
              <div
              ...
              <p className="text-sm text-[#8A8D93] leading-relaxed">{f.desc}</p>
            </div>
          );
        })}
      </div>
      ...
      {/* CTA Panel */}
      <div className="rounded-[22px] p-10 md:p-14 relative overflow-hidden" style={{ background: 'rgba(22, 22, 26, 0.6)', backdropFilter: 'blur(24px)', color: '#fff', border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 24px 70px rgba(0,0,0,0.25)' }}>
        {/* Background decoration */}
        <div className="absolute top-0 right-0 w-64 h-64 rounded-full opacity-15"
          style={{ background: 'radial-gradient(circle, #00F0FF 0%, transparent 70%)', transform: 'translate(30%, -30%)' }} />
        <div className="absolute bottom-0 left-0 w-48 h-48 rounded-full opacity-15"
          style={{ background: 'radial-gradient(circle, #0044FF 0%, transparent 70%)', transform: 'translate(-30%, 30%)' }} />

        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          <div>
            <span className="text-xs font-mono text-[#00F0FF] font-bold uppercase tracking-widest mb-3 block">
              FREE SEO AUDIT
            </span>
          ...
          <div className="flex flex-col gap-4">
            ...
            <a
              href="#contact"
              className="inline-flex items-center gap-2 mt-2 bg-[#00F0FF] text-white font-bold text-sm px-7 py-3.5 rounded-full hover:bg-purple-500 hover:scale-105 active:scale-95 transition-all duration-200 shadow-lg w-fit"
            >
